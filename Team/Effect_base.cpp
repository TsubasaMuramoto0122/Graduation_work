...
void CEffect_base::SetPosField(D3DXVECTOR3 pos, D3DXVECTOR3 Size, float Rotate, float Rotate2)
{
	...
	//頂点座標の設定
	pVtx[0].pos = D3DXVECTOR3(pos.x - pos.x + (cosf(Rotate2)) * Size.x, Size.y, pos.z - pos.z + (sinf(Rotate))* Size.x);
	pVtx[1].pos = D3DXVECTOR3(pos.x - pos.x + (sinf(Rotate))  * Size.x, Size.y, pos.z - pos.z - (cosf(Rotate2))* Size.x);
	pVtx[2].pos = D3DXVECTOR3(pos.x - pos.x - (sinf(Rotate))  * Size.x, Size.y, pos.z - pos.z + (cosf(Rotate2))* Size.x);
	pVtx[3].pos = D3DXVECTOR3(pos.x - pos.x - (cosf(Rotate2)) * Size.x, Size.y, pos.z - pos.z - (sinf(Rotate))* Size.x);
