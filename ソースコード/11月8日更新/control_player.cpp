...
void CControlPlayer::Attack(CPlayer *pPlayer)
{
	...
	else if (m_bAttack == true)
	{
		...
		if (m_nAttackCount == 15)
		{
			...
			m_pCollision = CCollisionSphere::Create(D3DXVECTOR3(pos.x, pos.y + pPlayer->GetRadius(), pos.z), pPlayer->GetRadius() * 2.5f, 16, 16, CCollisionSphere::COLLISION_S_TYPE::COLLISION_S_TYPE_ATTACK, PLAYER_ATTACK_TIME);
